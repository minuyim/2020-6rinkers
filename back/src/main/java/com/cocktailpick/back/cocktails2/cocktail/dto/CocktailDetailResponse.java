...
import com.cocktailpick.back.cocktails2.recipe.dto.RecipeItemResponse;
	...
	public static List<CocktailDetailResponse> listOf(List<Cocktail> recommend, Favorites favorites) {
		...
		for (Cocktail cocktail : recommend) {
			if (favorites.isContainCocktail(cocktail)) {
				cocktailDetailResponses.add(
					CocktailDetailResponse.of(cocktail, true));
			} else {
				cocktailDetailResponses.add(
					CocktailDetailResponse.of(cocktail, false));
			}
		}
